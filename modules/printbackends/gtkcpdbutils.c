...
typedef void (*GtkCpdbRequestStateFunc) (GtkCpdbRequest *request);

static void _connect (GtkCpdbRequest *request);
static void _post_send (GtkCpdbRequest *request);
static void _post_write_request (GtkCpdbRequest *request);
static void _post_write_data (GtkCpdbRequest *request);
static void _post_check (GtkCpdbRequest *request);
static void _post_auth (GtkCpdbRequest *request);
static void _post_read_response (GtkCpdbRequest *request);

static void _get_send (GtkCpdbRequest *request);
static void _get_check (GtkCpdbRequest *request);
static void _get_auth (GtkCpdbRequest *request);
static void _get_read_data (GtkCpdbRequest *request);
...
static GtkCpdbRequestStateFunc post_states[] = {
  _connect,
  _post_send,
  _post_write_request,
  _post_write_data,
...
static GtkCpdbRequestStateFunc get_states[] = {
  _connect,
  _get_send,
  _get_check,
  _get_auth,
...
static void G_GNUC_PRINTF (5, 6)
    gtk_cpdb_result_set_error (GtkCpdbResult *result,
...
GtkCpdbRequest *
gtk_cpdb_request_new (http_t *connection,
                      GtkCpdbRequestType req_type,
                      int operation_id,
                      GIOChannel *data_io,
                      const char *server,
                      const char *resource)
{
  return gtk_cpdb_request_new_with_username (connection,
                                             req_type,
                                             operation_id,
                                             data_io,
                                             server,
...
void
gtk_cpdb_request_free (GtkCpdbRequest *request)
{
  if (request->own_http)
    {
      if (request->http)
  ...
  gtk_cpdb_result_free (request->result);

  g_free (request);
}

gboolean
gtk_cpdb_request_read_write (GtkCpdbRequest *request, gboolean connect_only)
{
  if (connect_only && request->state != GTK_CPDB_REQUEST_START)
    return FALSE;

  do
    {
      if (request->type == GTK_CPDB_POST)
        post_states[request->state](request);
      else if (request->type == GTK_CPDB_GET)
        get_states[request->state](request);

      if (gtk_cpdb_result_is_error (request->result))
        request->state = GTK_CPDB_REQUEST_DONE;

      if (request->attempts > _GTK_CPDB_MAX_ATTEMPTS &&
          request->state != GTK_CPDB_REQUEST_DONE)
        {
      ...
      if (request->state == GTK_CPDB_REQUEST_DONE)
        {
   ...
   * Cpdb has an internal read buffer. And if this buffer is filled, we may
   * never get a poll event again. */
  while (request->poll_state == GTK_CPDB_HTTP_READ && request->http && httpCheck (request->http));

  return FALSE;
}

GtkCpdbPollState
gtk_cpdb_request_get_poll_state (GtkCpdbRequest *request)
{
  return request->poll_state;
}

GtkCpdbResult *
gtk_cpdb_request_get_result (GtkCpdbRequest *request)
{
...
void
gtk_cpdb_request_ipp_add_strings (GtkCpdbRequest *request,
                                  ipp_tag_t group,
                                  ipp_tag_t tag,
                                  const char *name,
                                  int num_values,
...
static void
_connect (GtkCpdbRequest *request)
{
  request->poll_state = GTK_CPDB_HTTP_IDLE;
  ...
  if (request->http == NULL)
  ...
  else
    {
      ...
      /* we always write to the socket after we get
         the connection */
      request->poll_state = GTK_CPDB_HTTP_WRITE;
    }
}

static void
_post_send (GtkCpdbRequest *request)
{
  char length[255];
  ...
  request->state = GTK_CPDB_POST_WRITE_REQUEST;
  ippSetState (request->ipp_request, IPP_IDLE);
}

static void
_post_write_request (GtkCpdbRequest *request)
{
  ...
  if (ipp_status == IPP_DATA)
    {
      if (request->data_io != NULL)
        request->state = GTK_CPDB_POST_WRITE_DATA;
      else
        {
          request->state = GTK_CPDB_POST_CHECK;
          request->poll_state = GTK_CPDB_HTTP_READ;
        }
    }
}

static void
_post_write_data (GtkCpdbRequest *request)
{
  gsize bytes;
  char buffer[_GTK_CPDB_MAX_CHUNK_SIZE];
...
static void
_post_auth (GtkCpdbRequest *request)
{
  if (request->password_state == GTK_CPDB_PASSWORD_HAS)
    {
      if (request->password == NULL)
        {
          request->state = GTK_CPDB_POST_DONE;
...
static void
_get_auth (GtkCpdbRequest *request)
{
  if (request->password_state == GTK_CPDB_PASSWORD_HAS)
    {
      if (request->password == NULL)
        {
          request->state = GTK_CPDB_GET_DONE;
          request->poll_state = GTK_CPDB_HTTP_IDLE;
        ...
        request->state = GTK_CPDB_GET_CHECK;
    }
}

/* Very ugly hack: cpdb has a stupid synchronous password callback 
 * that doesn't even take the request or user data parameters, so 
 * we have to use a static variable to pass the password to it.
 * Not threadsafe !
 * The callback sets cpdb_password to NULL to signal that the 
 * password has been used.
 */
static char *cpdb_password = NULL;
static char *cpdb_username = NULL;
...
static void
_post_check (GtkCpdbRequest *request)
{
  http_status_t http_status;
  ...
  GTK_NOTE (PRINTING,
            g_print ("CPDB Backend: %s - status %i\n", G_STRFUNC, http_status));
  ...
  if (http_status == HTTP_CONTINUE)
  ...
  else if (http_status == HTTP_UNAUTHORIZED)
    {
      ...
      if (request->password_state == GTK_CPDB_PASSWORD_APPLIED)
        {
          request->poll_state = GTK_CPDB_HTTP_IDLE;
          request->password_state = GTK_CPDB_PASSWORD_NOT_VALID;
          request->state = GTK_CPDB_POST_AUTH;
      ...
      else
        {
          if (request->password_state == GTK_CPDB_PASSWORD_NONE)
          ...
          else
            {
              ...
              request->password_state = GTK_CPDB_PASSWORD_APPLIED;
            }
        }
    ...
    {
      request->state = GTK_CPDB_POST_READ_RESPONSE;
      return;
    }
...
static void
_post_read_response (GtkCpdbRequest *request)
{
  ipp_state_t ipp_status;
...
static void
_get_send (GtkCpdbRequest *request)
{
  GTK_NOTE (PRINTING,
            g_print ("CPDB Backend: %s\n", G_STRFUNC));
...
static void
_get_check (GtkCpdbRequest *request)
{
  http_status_t http_status;
  ...
  if (http_status == HTTP_CONTINUE)
      ...
      if (request->password_state == GTK_CPDB_PASSWORD_APPLIED)
        {
          request->poll_state = GTK_CPDB_HTTP_IDLE;
          request->password_state = GTK_CPDB_PASSWORD_NOT_VALID;
          request->state = GTK_CPDB_GET_AUTH;
          request->need_password = TRUE;
    ...
    {
      request->state = GTK_CPDB_GET_READ_DATA;
      return;
    }
...
static void
_get_read_data (GtkCpdbRequest *request)
{
  char buffer[_GTK_CPDB_MAX_CHUNK_SIZE];
  gsize bytes;
  gsize bytes_written;
  ...
  GTK_NOTE (PRINTING,
            g_print ("CPDB Backend: %" G_GSIZE_FORMAT " bytes read\n", bytes));
...
gboolean
gtk_cpdb_request_is_done (GtkCpdbRequest *request)
{
  return (request->state == GTK_CPDB_REQUEST_DONE);
}

gboolean
gtk_cpdb_result_is_error (GtkCpdbResult *result)
{
  return result->is_error;
}

ipp_t *
gtk_cpdb_result_get_response (GtkCpdbResult *result)
{
  return result->ipp_response;
}

GtkCpdbErrorType
gtk_cpdb_result_get_error_type (GtkCpdbResult *result)
{
  return result->error_type;
}

int
gtk_cpdb_result_get_error_status (GtkCpdbResult *result)
{
  return result->error_status;
}

int
gtk_cpdb_result_get_error_code (GtkCpdbResult *result)
{
  return result->error_code;
}

const char *
gtk_cpdb_result_get_error_string (GtkCpdbResult *result)
{
  return result->error_msg;
}

/* This function allocates new instance of GtkCpdbConnectionTest() and creates
 * a socket for communication with a CPDB server 'server'.
 */
GtkCpdbConnectionTest *
gtk_cpdb_connection_test_new (const char *server,
                              const int port)
{
  GtkCpdbConnectionTest *result = NULL;
  char *port_str = NULL;

  result = g_new (GtkCpdbConnectionTest, 1);
  ...
  result->at_init = GTK_CPDB_CONNECTION_NOT_AVAILABLE;

  result->at_init = gtk_cpdb_connection_test_get_state (result);

  return result;
}

/* A non-blocking test whether it is possible to connect to a CPDB server specified
 * inside of GtkCpdbConnectionTest structure.
 *  - you need to check it more then once.
 * The connection is closed after a successful connection.
 */
GtkCpdbConnectionState
gtk_cpdb_connection_test_get_state (GtkCpdbConnectionTest *test)
{
  GtkCpdbConnectionState result = GTK_CPDB_CONNECTION_NOT_AVAILABLE;
  ...
  if (test == NULL)
    return GTK_CPDB_CONNECTION_NOT_AVAILABLE;

  if (test->at_init == GTK_CPDB_CONNECTION_AVAILABLE)
    {
      test->at_init = GTK_CPDB_CONNECTION_NOT_AVAILABLE;
      return GTK_CPDB_CONNECTION_AVAILABLE;
    }
  else
    {
      if (test->socket == -1)
          ...
          if (code == 0 || error_code == EISCONN)
            {
              ...
              result = GTK_CPDB_CONNECTION_AVAILABLE;
            }
          else
            {
              if (error_code == EALREADY || error_code == EINPROGRESS)
                result = GTK_CPDB_CONNECTION_IN_PROGRESS;
              else
                {
                  close (test->socket);
                  test->socket = -1;
                  test->last_wrong_addr = test->current_addr;
                  result = GTK_CPDB_CONNECTION_NOT_AVAILABLE;
                }
            }
        }
 ...
 */
void
gtk_cpdb_connection_test_free (GtkCpdbConnectionTest *test)
{
  if (test == NULL)
    return;
